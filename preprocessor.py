...
class Preprocessor:
  ...
  def _thresholdImage(self) -> None:
    ...
    thresh = cv2.adaptiveThreshold(self._imgray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
  ...
  def detect_rectangle(self) -> list: #list of tuples
    ...
    for cnt in contours:
      #x1, y1 = cnt[0][0]
      approx = cv2.approxPolyDP(cnt, 0.01*cv2.arcLength(cnt, True), True) # detects poly
      if len(approx) == 4:
